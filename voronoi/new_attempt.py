...
def pick_point(event):
    if select_allowed and event.artist == sphere:
        print("picking")
        ind = event.ind
        global xs = np.take(x_arr, ind)
        global ys = np.take(y_arr, ind)
        global zs = np.take(z_arr, ind)
        print(xs)
        print(ys)
        print(zs)
...
def on_release(event):
    ...
    print(select_allowed)
    print(xs)
    if select_allowed and event.inaxes == ax and xs: # xs not None means this occurs after a pick event
        print("narrowing")
        try:
        ...
        mouse_x, mouse_y, mouse_z = coord_list
        mouse = Point3D(mouse_x, mouse_y, mouse_z)
        # select picked point closest to current mouse location
        distance = float('inf')
        x, y, z = None, None, None
        for i in range(len(xs)):
            xi, yi, zi = xs[i], ys[i], zs[i]
            distance_i = mouse.distance(Point3D(xi, yi, zi))
            if distance_i < distance:
                distance = distance_i
                x, y, z = xi, yi, zi
        # x, y, z now contain picked point closest to mouse location
...
if __name__ == '__main__':
    ...
    xs, ys, zs = None, None, None # holder for point picking

#    ax.plot_surface(scale*x, scale*y, scale*z, color='w', zorder = 0)
    sphere = ax.plot_surface(x_arr,y_arr,z_arr, alpha=.5, picker = True)
